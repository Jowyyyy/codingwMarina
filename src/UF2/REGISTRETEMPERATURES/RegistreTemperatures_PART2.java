...
public class RegistreTemperatures_PART2 {
  ...
  public void calculaDiferencia() {
    int num_mayor = temperatures[0];
    int num_menor = temperatures[0];
    for (int k = 1;k<numTemperatures;k++) {
      if (temperatures[k] > num_mayor) {
        num_mayor=temperatures[k];
      }

      else if (temperatures[k] < num_menor) {
        num_menor=temperatures[k];
      }
  }
      System.out.print("El numero mes gran es " +num_mayor+ "i el mes petit es "+num_menor);
